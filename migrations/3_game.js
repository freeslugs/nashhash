module.exports = (deployer, network, accounts) => {
	...
	deployer.deploy(GameRegistry).then(async () => {
		...
		await gameRegistry.insert(TwoThirds1.address);
	    ...
	    await gameRegistry.insert(TwoThirds01.address);
		gameAddresses.push(TwoThirds01.address);
		await npt.addMinter(TwoThirds01.address);
	    ...
	    await gameRegistry.insert(TwoThirds001.address);
	    gameAddresses.push(TwoThirds001.address);
	    await npt.addMinter(TwoThirds001.address);
		...
		const LUN1 = await LowestUniqueNum.new(
	   	...
	   	await gameRegistry.insert(LUN1.address);
		gameAddresses.push(LUN1.address);
		await npt.addMinter(LUN1.address);
	    ...
	    await gameRegistry.insert(LUN01.address);
	 	gameAddresses.push(LUN01.address);
		await npt.addMinter(LUN01.address);
	    ...
	    await gameRegistry.insert(LUN001.address);
	    gameAddresses.push(LUN001.address);
	    await npt.addMinter(LUN001.address);
	})
};
